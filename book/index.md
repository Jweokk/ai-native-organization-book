# AI 原生组织（AI-Native Organization）

> 免费公开 · 持续更新 · 欢迎在线阅读与分享

**作者：Jweokk** · 联系邮箱：weokk2025@gmail.com

<div style="text-align:center; margin: 2em 0;">
<a class="md-button md-button--primary" href="ai-native-organization.pdf">📥 下载整本 PDF（每周更新，最新版）</a>
</div>

---

## 关于本书

模型已经不稀缺了，**能把模型长进组织里的人，才稀缺**。

过去两年，企业的 AI 项目大多以两种方式收场：要么 95% 的投入打水漂（MIT NANDA《The GenAI Divide》），要么工具买了、培训做了、激励给了，年终复盘使用率不到 15%。与此同时，一批组织——从 DeepSeek 的 160 人创新密度，到 Anthropic 的 6 步转型框架，到传神的"能量金"激励——证明了另一条路：**让组织本身长出 AI 基因，而不是给传统组织贴 AI 的膏药**。

这本书是把"AI 原生组织"这个正在发生但还没有名字的系统性变化，完整地研究一遍。它聊清楚三件事：

- **是什么**：AI 原生组织与"AI 转型"的本质区别，贴膏药 vs 长基因
- **证据**：HBS/INSEAD/WEF/腾讯研究院的实证研究怎么看
- **怎么做**：组织形态重构、工作流重构、激励重构，以及从 0 到 1 的行动手册

书里所有数据和案例，都在附录 A 里标明了出处。

## 这本书是怎么写出来的

这本书是一本"活书"：一个自动化系统每天扫描全球 AI 落地/组织变革信息，提炼后存入知识库；每周自动把新增内容合并进对应章节，版本号递增（v1.0.0 → v1.0.1 → ...）。版本历史见[附录 C](14-附录C-版本历史与更新说明.md)与 [CHANGELOG](https://github.com/Jweokk/ai-native-organization-book/blob/main/CHANGELOG.md)。

## 目录

- [自序](00-自序.md) — 为什么写这本书 + 这本书是怎么自动写出来的
- [第 1 章](01-第1章-AI原生组织的崛起.md) — AI 原生组织的崛起
- [第 2 章](02-第2章-什么是AI原生组织.md) — 什么是 AI 原生组织
- [第 3 章](03-第3章-证据-实证研究怎么看.md) — 证据：实证研究怎么看
- [第 4 章](04-第4章-组织形态重构.md) — 组织形态重构
- [第 5 章](05-第5章-工作流与决策重构.md) — 工作流与决策重构
- [第 6 章](06-第6章-激励与人才.md) — 激励与人才
- [第 7 章](07-第7章-案例集.md) — 案例集：全球与中国的 AI 原生组织
- [第 8 章](08-第8章-打造你的AI原生组织.md) — 打造你的 AI 原生组织
- [第 9 章](09-第9章-挑战风险与未来.md) — 挑战、风险与未来
- [第 10 章](10-第10章-行业篇.md) — 行业篇：十个行业的 AI 原生转型
- [后记](11-后记.md)
- [附录 A](12-附录A-案例索引与资料出处.md) — 案例索引与资料出处
- [附录 B](13-附录B-常用指标.md) — 常用指标
- [附录 C](14-附录C-版本历史与更新说明.md) — 版本历史与更新说明

## 同一作者的其他书

本书是 Jweokk 作品集中的一册。作者的更多开源书与开源项目，欢迎逛逛 [Jweokk 作品集](https://ai.fly2ai.top)——全部免费公开：

- **《AI 时代人生指南》**（[在线阅读](https://ailife.fly2ai.top) · [GitHub](https://github.com/Jweokk/ai-life-guide)）：AI 时代，人该怎么活？
- **《AI 客户经营》**（[在线阅读](https://ai-customer-growth.fly2ai.top) · [GitHub](https://github.com/Jweokk/ai-customer-growth-book)）：AI 如何重塑客户从开拓到留存的全生命周期
- **《AI 获客》**（[在线阅读](https://ai-customer-acquisition.fly2ai.top) · [GitHub](https://github.com/Jweokk/ai-customer-acquisition-book)）：用 AI 拓客——把 AI 时代的新入口占住，也把老办法用 AI 跑快

## 致谢

本书的组织形式与公开方式，受到范冰（XDash）《[前线部署工程师](https://github.com/xdash/FDE-the-Guidance-Book-of-Forward-Deployed-Engineer)》的启发——免费公开全文、GitHub 仓库 + 官网阅读 + PDF 整本下载、附录标注出处的模式，是一个值得推广的开源知识实践。特此致谢。

## 版权声明

本书著作权归作者 Jweokk 所有，供读者**免费阅读与非商业性分享**，转载请务必注明出处与作者；任何商业用途（包括但不限于出版、培训、付费内容改编）须事先获得作者书面许可（weokk2025@gmail.com）。
